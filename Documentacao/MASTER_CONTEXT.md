...
**Compartilhar horários (`js-btn-compartilhar-whatsapp`/`js-whatsapp-enviar`, `js/agenda.js`):** modal com Dias (multi-seleção, como já era), **Duração do atendimento** (chips: "Não definir" + `gerarOpcoesDuracao`) e **Mostrar** (Apenas livres / Livres + encaixes / Apenas encaixes).
...
2. **Duração "Não definir"**: todos os horários "livre" da grade entram na lista de disponíveis, sem cálculo de passo (`todosLivresDoDia`).
3. **Duração escolhida** (`calcularLivresEsobrasDoDia(iso, duracao)`): dentro de cada trecho contínuo, anda em passos de `duracao` a partir do início do trecho — só inclui um horário se a duração **cabe inteira** antes do fim do trecho (regra de encaixe estrito, validada com o usuário num exemplo real de 17:30 numa janela 14:30–18:00 pedindo 60min: para às 16:30, não em 17:30). A sobra do trecho que não cabe vira **"encaixe"** só pra aquele compartilhamento (mesclada com os encaixes reais da grade, `todosEncaixesDoDia`) — não muda a Agenda em si, nem o que aparece nela.
4. "Mostrar" decide o que entra na mensagem final por dia: só a lista de livres, só a de encaixes (crua, sem filtro de duração), ou ambas.
...
| Chave | Conteúdo |
|---|---|
| `agendaV3:config` | tema, corPrincipal, horaInicio, horaFim, intervaloGrade, tempoPadraoAtendimento (sugestão opcional, pode ser `null`), assinaturaStatus |
...
~~`inicializarDadosFicticios()` populava cada chave com dados fictícios~~ — **removida em 2026-07-01** (ver seção 17); hoje só `agendaV3:config`/`agendaV3:whatsapp` recebem valores de fábrica (`seedConfig()`/`seedWhatsapp()`), aplicados uma vez na conclusão do Onboarding.
...
- **Agendamento** (a entidade mais importante): `{id, data, hora, clienteId?, nomeCliente, servicosIds, observacao, status: "agendado"|"realizado_pago"|"realizado_pendente"|"cancelado"|"bloqueado", realizadoEm?, valorTotal?, pago?, pagamentos?, valorPendente?, nomeBloqueio?, duracaoMinutos?}` — **`duracaoMinutos` (opcional, adicionado em 2026-07-04):** substitui o antigo `tempoPadraoAtendimento` global como base do algoritmo de encaixe; escolhido por chip (`gerarOpcoesDuracao`) ao criar/editar o agendamento, sugerindo `tempoPadraoAtendimento` quando definido. Ver seção 12.
...
- Configurações: horários/grade/duração padrão reais; Limpar agenda / Apagar todos os dados / Redefinir onboarding funcionais (Etapa 5). **Reformulado em 2026-07-01, ver seção abaixo — dados fictícios removidos de vez.** **"Compartilhamento da agenda (Simples/Estratégico)" removido e "Tempo padrão de atendimento" renomeado pra "Duração padrão (opcional)" em 2026-07-04 — ver rodada abaixo.**
...
- **Duração por agendamento + novo Compartilhar horários (Fase 4, 2026-07-04):** modal Novo/Editar agendamento ganhou campo "Duração" (chips via `gerarOpcoesDuracao`, sugerindo `tempoPadraoAtendimento` quando definido). Modal "Compartilhar horários" reescrito: ganhou "Duração do atendimento" (com "Não definir") e "Mostrar" (Apenas livres / Livres + encaixes / Apenas encaixes), usando o algoritmo de trechos contínuos + passo (seção 12). Zero impacto em Relatório/Pendentes/Ranking/Cliente-detalhe — todos leem `agendaV3:agendamentos` direto por data/status, sem passar pela grade/duração.
