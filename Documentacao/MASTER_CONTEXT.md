...
8. **Preservação de horários fora do padrão atual (corrigido em 2026-07-01):** a lista de horários do dia não é só a grade gerada pelo `intervaloGrade` atual — é a **união** dessa grade com todo horário que já tem um compromisso/bloqueio real (`horariosParaExibir` em `classificarGradeDoDia`). Isso importa porque o usuário pode mudar `intervaloGrade`/`horaInicio`/`horaFim`/`tempoPadraoAtendimento` livremente depois de já ter agendamentos reais — sem essa união, um agendamento antigo num horário que não bate mais com a grade nova (ex.: 09:30 numa grade que virou de 20 em 20 minutos) simplesmente não aparecia na lista, embora continuasse existindo nos dados (Relatório/Pendentes/Ranking não são afetados, pois leem `agendaV3:agendamentos` direto por data/status, sem passar pela grade). Testado trocando `intervaloGrade` de 30 pra 20 com um agendamento fixo às 09:30 — continua aparecendo, classificado corretamente como "agendado". Os modais "Primeiro horário"/"Último horário"/"Visualização"/"Tempo padrão" em Configurações ganharam um aviso curto avisando disso ao usuário.
9. **Mais opções de grade/tempo padrão (2026-07-01):** "Visualização" (`intervaloGrade`) ganhou 5/10/20/25/35/40/45/50 min (além dos 15/30/60 já existentes — lista completa agora 5 a 60, de 5 em 5 a partir do 5 até o 25, depois 5 em 5 até 60). "Tempo padrão de atendimento" ganhou 20/35/40/50/75 min (além de 15/30/45/60/90/120). Ambos os modais em `configuracoes.html` ganharam `max-height:50vh;overflow-y:auto` (mesmo padrão já usado nos modais de Primeiro/Último horário) pra acomodar as 11 opções cada sem quebrar o layout. **Chips continuam sendo o padrão pra esse tipo de escolha única** — decisão explícita de manter consistência com o resto do app (Primeiro/Último horário e o histórico de Intervalos, que já teve um seletor de horário customizado e voltou pra chips).
...
- Configurações: horários/grade/tempo padrão/modo de compartilhamento reais; Limpar agenda / Apagar todos os dados / Redefinir onboarding funcionais (Etapa 5). **Reformulado em 2026-07-01, ver seção abaixo — dados fictícios removidos de vez.**
