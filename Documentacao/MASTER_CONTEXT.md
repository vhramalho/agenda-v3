...
- **Bug de bfcache corrigido (2026-07-01):** `voltarOuInicio()` (`js/menu.js`) usa `window.history.back()` — quando o navegador restaura a página anterior do **bfcache** (back-forward cache), nenhum script reexecuta, só o evento `pageshow` dispara (com `evento.persisted === true`). Qualquer efeito que só roda uma vez no carregamento (como `aplicarTema()` em `js/tema.js`, chamado solto no fim do arquivo) fica desatualizado se o usuário mudou algo (ex. tema/cor) em outra aba/página antes de voltar. Corrigido com `window.addEventListener("pageshow", (e) => { if (e.persisted) aplicarTema(); })`. **Vale o mesmo princípio pra qualquer novo estado global aplicado via JS puro no load** — se something semelhante for adicionado no futuro, lembrar de reagir a `pageshow`/`persisted`, não só ao carregamento inicial.
